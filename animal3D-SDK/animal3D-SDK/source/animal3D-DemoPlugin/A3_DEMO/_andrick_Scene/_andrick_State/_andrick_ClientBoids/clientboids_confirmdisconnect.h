#ifndef CLIENTBOIDS_CONFIRMDISCONNECT_H_
#define CLIENTBOIDS_CONFIRMDISCONNECT_H_

#include <A3_DEMO/_andrick_Scene/_andrick_State/andrick_scenestate.h>

class ClientBoidsConfirmDisconnect : public SceneState
{
public:
	ClientBoidsConfirmDisconnect(std::shared_ptr<Scene> parentScene);
	virtual ~ClientBoidsConfirmDisconnect() = default;

	virtual void enteringState() override;
	virtual void processInput() override;
	virtual void processIncomingEvent(std::shared_ptr<Event> evnt) override;
	virtual void update() override;
	virtual void queueOutgoingEvents() override;
	virtual void render() override;
	virtual void exitingState() override;
};

#endif